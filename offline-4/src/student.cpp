...
offline_4::student::student(const size_t &id) : individual(id)
{
...
void offline_4::student::action()
{
    ...
    std::osyncstream(std::cout) << "Student " << get_id() << " has arrived at the print station at time " << offline_4::timer::get_microseconds_count() << std::endl;
    ...
    std::osyncstream(std::cout) << "Student " << get_id() << " has finished printing at time " << offline_4::timer::get_microseconds_count() << std::endl;
    ...
    std::osyncstream(std::cout) << "Group " << get_group()->get_id() << " has finished printing at time " << offline_4::timer::get_microseconds_count() << std::endl;

    student::binder_semaphore.acquire();

    std::osyncstream(std::cout) << "Group " << get_group()->get_id() << " has started binding at time " << offline_4::timer::get_microseconds_count() << std::endl;

    sleep(student::bind_delay);
    student::binder_semaphore.release();

    std::osyncstream(std::cout) << "Group " << get_group()->get_id() << " has finished binding at time " << offline_4::timer::get_microseconds_count() << std::endl;

    offline_4::stuff::add_submission();

    std::osyncstream(std::cout) << "Group " << get_group()->get_id() << " has submitted the report at time " << offline_4::timer::get_microseconds_count() << std::endl;
}
